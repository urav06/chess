from  engine import Game,Board,Color
...
class  SearchTree:
  ...
  def search_and_evaluation(self,game:Game,depth:int,state:bool): # state is 0 for max and 1 for min
      if depth==0:
          ...
          for (i,current_move) in enumerate(all_legal_moves):
                  ...
                  value=self.search_and_evaluation(game.seek_move(current_move),depth-1,state= not state)
                  if state:
                    if  value >self.alpha:
                      break
                    else:
                    ...
                    if value < self.beta:
                      break
                       ...
                       self.beta = value 
                  out_value =  max(out_value,value) if state else min(out_value,value)
